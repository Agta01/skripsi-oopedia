    ...
    <div class="container-fluid position-relative">
            ...
            @auth
                <div class="d-none d-lg-flex align-items-center">
                    <h6 class="mb-0 fw-bold d-flex align-items-center"
                        style="color: #475569; font-size: 14.5px; letter-spacing: 0.3px;">
                        <span
                            style="display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 8px; background: rgba(59, 130, 246, 0.1); color: #3b82f6; margin-right: 12px;">
        ...
        @else
            <div class="flex-grow-1 d-none d-md-flex justify-content-center px-4">
                <form
                    action="{{ request()->routeIs('mahasiswa.materials.questions*') ? route('mahasiswa.materials.questions.index') : route('mahasiswa.materials.index') }}"
                    method="GET" class="search-wrapper m-0" style="position: relative;">
                    <input type="text" name="search" id="navbarSearchInput" class="search-input"
                        placeholder="Cari materi atau latihan soal..." value="{{ request('search') }}" autocomplete="off">
                    <i class="fas fa-search search-icon"></i>
                    <button type="submit" class="d-none"></button>

                    <!-- Suggestions Dropdown UI -->
                    <div id="searchSuggestionsContainer"
                        style="display: none; position: absolute; top: calc(100% + 5px); left: 0; right: 0; background: white; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 1px solid rgba(0,0,0,0.05); overflow: hidden; z-index: 1000;">
                        <ul id="searchSuggestionsList" class="list-unstyled mb-0"
        ...
        <div class="d-flex align-items-center justify-content-end gap-2 gap-md-3"
            style="{{ $isGuest ? '' : 'width: 250px;' }}">
            @guest
                <a href="{{ route('login') }}" class="btn btn-outline-primary rounded-pill px-4"
                    style="font-weight: 500; font-size: 14px;">Masuk</a>
                <a href="{{ route('register') }}" class="btn btn-primary rounded-pill px-4"
                    style="font-weight: 500; font-size: 14px; box-shadow: 0 4px 12px rgba(13, 110, 253, 0.2);">Daftar</a>
            @endguest
            ...
            @auth
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end p-0" aria-labelledby="notifDropdown"
                        ...
                        <div class="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
                            ...
                            @if(auth()->user()->unreadNotifications->count() > 0)
                                <span class="badge bg-primary rounded-pill">{{ auth()->user()->unreadNotifications->count() }}
                                    Baru</span>
                            @endif
                        </div>
                        ...
                        <div class="p-2 border-top text-center text-sm">
                            <a href="#" class="text-primary fw-medium text-decoration-none" style="font-size: 13px;">Tandai
                                semua dibaca</a>
                        </div>
                    </div>
                </div>
                ...
                <!-- Profile Dropdown -->
                <div class="dropdown">
                    <button class="profile-toggle" type="button" id="profileDropdown" data-bs-toggle="dropdown"
                        aria-expanded="false">
                        @php
                            $initials = collect(explode(' ', $userName))->map(function ($word) {
                                return strtoupper(substr($word, 0, 1)); })->take(2)->implode('');
                        @endphp
